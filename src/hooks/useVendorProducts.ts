...
export const useVendorProducts = (statusFilter?: string) => {
    ...
    const deleteProduct = async (productId: string): Promise<boolean> => {
        ...
        try {
            ...
            // This prevents FK constraint violation with order_items
            const { error } = await supabase
                .from('products')
            ...
            if (error) {
                ...
                toast.error('فشل في أرشفة المنتج: ' + error.message);
                return false;
            }
            ...
            toast.success('تم أرشفة المنتج بنجاح', {
                description: 'المنتج لن يظهر في المتجر بعد الآن'
            });
            return true;
        } catch (error) {
...
export const useAdminProducts = (vendorFilter?: string, statusFilter?: string) => {
    ...
    const deleteProduct = async (productId: string): Promise<boolean> => {
        if (!user || !isAdmin) {
            toast.error('ليس لديك صلاحية لأرشفة المنتج');
            return false;
        }
